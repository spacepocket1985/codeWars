...
function isLanguageDiverse(list) {
  let countJsRubyPythonLangArr = [0, 0, 0];
  list.forEach((item) =>
    item.language === 'JavaScript'
      ? ++countJsRubyPythonLangArr[0]
      : item.language === 'Ruby'
      ? ++countJsRubyPythonLangArr[1]
      : ++countJsRubyPythonLangArr[2]
  );
  countJsRubyPythonLangArr.sort((a, b) => b - a);
  return (
    Math.max(...countJsRubyPythonLangArr) /
      Math.min(...countJsRubyPythonLangArr) <
    2
  );
}
