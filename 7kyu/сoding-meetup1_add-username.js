...
function addUsername(list) {
  return list.map((item) => {
    console.log(new Date().getFullYear() - item.age)
    const nickNameDate = new Date().getFullYear() - item.age;
    item.username = `${item.firstName.toLowerCase()}${item.lastName[0].toLowerCase()}${nickNameDate}`;
    return item;
  });
}
